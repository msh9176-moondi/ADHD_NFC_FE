...
function KakaoCallbackPage() {
  ...
  useEffect(() => {
    ...
    const handleKakaoCallback = async () => {
      try {
        const response = await api.post<KakaoCallbackResponse>("/auth/kakao/callback", { code });
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="text-[#795549] text-lg">
          {step === "loading" ? "카카오 로그인 처리 중..." : "회원가입 처리 중..."}
        </div>
      ...
      <div className="w-full flex flex-col items-center">
        ...
        <div className="w-full mb-6 p-4 bg-[#F5F5F5] rounded-2xl text-center">
          <p className="text-[#795549] text-sm">
            <span className="font-bold">카카오</span> 계정으로 회원가입합니다.
          </p>
